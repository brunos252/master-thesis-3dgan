...
class net_G(torch.nn.Module):
    def __init__(self, args):
        ...
        self.layer2 = self.conv_layer(self.f_dim*16, self.f_dim*8, kernel_size=4, stride=2, padding=(1, 1, 1), bias=self.bias)
        self.layer3 = self.conv_layer(self.f_dim*8, self.f_dim*4, kernel_size=4, stride=2, padding=(1, 1, 1), bias=self.bias)
        self.layer4 = self.conv_layer(self.f_dim*4, self.f_dim*2, kernel_size=4, stride=2, padding=(1, 1, 1), bias=self.bias)
...
class net_D(torch.nn.Module):
    def __init__(self, args):
        ...
        self.layer6 = torch.nn.Sequential(
            torch.nn.Conv3d(self.f_dim*16, 1, kernel_size=4, stride=2, bias=self.bias, padding=padd),
            torch.nn.Sigmoid()
        )
